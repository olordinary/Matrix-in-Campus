...
void WebRTCManager::startLocalMedia()
{
    ...
    try {
        ...
        // 这里只是标记媒体已启动，实际的音视频采集需要配合其他库
        // 例如：FFmpeg、GStreamer、Qt Multimedia等

        qDebug() << "Local media started";
        m_mediaStarted = true;
        emit localStreamReady();
        emit statusChanged("本地媒体已启动");
    }
    catch (const std::exception &e) {
        qWarning() << "Failed to start local media:" << e.what();
        emit errorOccurred(QString("启动本地媒体失败: %1").arg(e.what()));
    }
}
...
void WebRTCManager::setupPeerConnection(bool isOfferer)
{
    ...
    if (m_pc) {
        qDebug() << "Peer connection already exists";
        return;
    }
